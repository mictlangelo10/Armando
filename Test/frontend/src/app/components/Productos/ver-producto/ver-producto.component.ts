  ...
  getProductos(): void {
    this.productoService.list().subscribe(
      (data) => {
        ...
        this.categorias = this.productos
          .map((objeto) => objeto.categoria)
  ...
  onDelete(id: number): void {
    Swal.fire({
      ...
      if (result.value) {
        ...
        Swal.fire('Cancelled', 'Client not deleted', 'error');
      }
    });
  }
  ...
  stock(id: number) {
    this.productoService.detail(id).subscribe(
      (data) => {
        this.producto = data;
        if (
          this.producto.existencia == true &&
          this.producto.porAgotarse == false
        ) {
          $('#stock' + id).css('background-color', '#9FE49A');
        } else if (
          this.producto.existencia == true &&
          this.producto.porAgotarse == true
        ) {
