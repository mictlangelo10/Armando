...
export class CheckInComponent implements OnInit {
      ...
      .subscribe(
        (res) => {
          ...
          this.empledadoService
            ...
            .subscribe((res) => {
              this.turno = res;
              console.log('Turn: ' + this.turno.turno);
              if (
                this.turno.turno == 'Morning' ||
                this.turno.turno == 'Evening'
              ) {
                console.log('The turn is Mat or Vesp');
                if (this.reviewCheckIn.length == 0) {
                  this.checkin.tipo = 'Checkin';
                  this.createCheckIn(this.checkin);
                } else if (this.reviewCheckIn.length == 1) {
                  this.checkin.tipo = 'Check-out';
                  this.createCheckIn(this.checkin);
                } else {
                  this.avisar();
                }
  ...
  openModal(): void {
    ...
    Swal.fire({
      ...
      html: '<p> Employee: ' + this.checkin.idEmpleado.toString() + '</p>',
      icon: 'success',
  ...
  avisar() {
    Swal.fire({
      title: 'All check-ins have been recorded',
      html: '<p> Employee: ' + this.checkin.idEmpleado.toString() + '</p>',
      icon: 'info',
      focusConfirm: false,
      confirmButtonText: 'OK',
